...
/**
 * @dev The official ThisIsFine smart contract
 */
contract ThisIsFine is Context, IERC20, Ownable {
    ...
    address public _pancakeswapV2LiquidityPair; // The address of the PancakeSwap V2 liquidity pairing for FINE/WBNB
    ...
    address payable public _marketingAddress =
        payable(0x20C53ed4B045608B1C47F560dADeAC0546b33c3F); // Marketing address used to pay for marketing
    address payable public _burnAddress =
        payable(0x000000000000000000000000000000000000dEaD); // Burn address used to burn a portion of tokens
    ...
    uint256 public _tokenSwapThreshold = 1 * 10**7 * 10**9; // 10 million

    // Timer Constants
    uint256 private constant DAY = 86400; // How many seconds in a day

    // Anti-Whale Settings
    uint256 public _whaleSellThreshold = 5 * 10**7 * 10**9; // 50 million
    uint256 public _whaleSellTimer = DAY; // 24 hours
    ...
    uint256 public _buybackUpperLimit = 1 * 10**5 * 10**9; // 100 thousand $FINE will be used to buy at maximum in any one trade
    ...
    uint256 public _lotteryChance = 25; // 1% chance to win on a buy
    uint256 public _lotteryThreshold = 1 * 10**3 * 10**9; // 1 thousand tokens required to be in the pool before lottery can be triggered
    uint256 public _lotteryMinimumSpend = 1 * 10**3 * 10**9; // 1 thousand tokens required to buy before before lottery can be triggered
    ...
    constructor() {
        ...
        IUniswapV2Router02 pancakeswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        );
        _pancakeswapV2LiquidityPair = IUniswapV2Factory(
    ...
    /**
     * @notice Required to recieve BNB from PancakeSwap V2 Router when swapping
     */
    ...
    /**
     * @notice Withdraws non-FINE tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function setRouterAddress(address router) public onlyOwner {
        ...
        address newPair = IUniswapV2Factory(newPancakeSwapRouter.factory())
            .getPair(address(this), newPancakeSwapRouter.WETH());
        if (newPair == address(0)) {
            newPair = IUniswapV2Factory(newPancakeSwapRouter.factory())
                .createPair(address(this), newPancakeSwapRouter.WETH());
        }
    ...
    function getTimeSinceFirstSell(address account)
        external
        view
        returns (uint256)
    {
    ...
    function setTokenSwapThreshold(uint256 tokenSwapThreshold)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            _enableLottery &&
            amount >= _lotteryMinimumSpend &&
            from == _pancakeswapV2LiquidityPair
        ) {
        ...
        // Gets the contracts FINE balance for buybacks, charity, liquidity and marketing
        ...
        if (
            _enableBuyback &&
            !currentlySwapping &&
            to == _pancakeswapV2LiquidityPair
        ) {
        ...
        if (takeFee && to == _pancakeswapV2LiquidityPair) {
            ...
            uint256 delta = block.timestamp.sub(_timeSinceFirstSell[from]);
    ...
    function _tokenTransfer(
        ...
        if (tFee > 0) {
            ...
            _takeTokens(
                tFee.sub(tPortion).sub(tPortion).sub(tPortion),
                tPortion
            );
        }
    ...
    /**
     * @notice Burns FINE tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of FINE tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from FINE)
